...
pygame.display.set_caption("Ingame Menü mit Pygame")
...
while running:
    time_delta = clock.tick(60) / 1000.0  # Delta-Zeit für FPS
    ...
    for event in pygame.event.get():
        ...
        # GUI-Manager Ereignisse weitergeben
        manager.process_events(event)

        # Prüfen, ob der Button geklickt wurde
        if event.type == pygame_gui.UI_BUTTON_PRESSED:
            if event.ui_element == button:
                print("Menü wird geschlossen...")
                running = False  # Beende das Programm (oder Menü ausblenden)

    # Hintergrundfarbe setzen
    screen.fill((30, 30, 30))

    # GUI-Manager updaten & zeichnen
